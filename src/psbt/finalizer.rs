...
use bitcoin::{self, Blockchain, PublicKey, Script};
...
pub fn finalize<C: secp256k1::Verification>(
    ...
    for index in 0..psbt.inputs.len() {
        ...
        let desc =
            get_descriptor(&psbt, index, chain).map_err(|e| Error::InputError(e, index))?;

        //generate the satisfaction witness and scriptsig
        let (witness, script_sig) = desc
            .get_satisfaction(PsbtInputSatisfier::new(&psbt, index), chain)
